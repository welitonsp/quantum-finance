...
- Branch principal: `main` — PRs #363–#387 mergeados. Working tree esperado: limpo. **Nenhum PR aberto.**
- Suíte: **1760 unit + 219 rules + 282 functions + 28 E2E** (+10 PR #387: wrappers MFA/TOTP).
...
- **Findings — estado da remediação:**
  ...
  - **M-01 (cobertura) — EM CAMPANHA (PRs #366+#368+#370+#372+#374+#376+#377+#379+#381+#383+#385+#387):** scope `src/lib/**` + `src/shared/lib/**` + 259 testes novos. Cobertura real (pós-#387): stmts ~68.67 / branches ~60.47 / funcs ~68.15 / lines ~72.09. Gates: stmts 67 / branches 60 / funcs 67 / lines 71. Meta final: branches→65 / lines→75. **⚠️ Nota #383:** o PR #382 ratchetou branches p/ 60 **acima** da real (~59.8%) → CI do `main` ficou vermelho; #383 destravou (59.76→60.04). #385 cardProjection+insightsEngine (→60.22); #387 wrappers MFA/TOTP (→60.47). **Margem 0.47%: ainda insuficiente p/ ratchet — acumular folga antes.** Próximos alvos: useRecurring, monthlyCashflow, scoreHistory (hooks, exigem renderHook + mock Firebase).
