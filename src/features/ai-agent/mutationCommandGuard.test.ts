...
describe('interpretMutationCommand', () => {
  ...
  it('refuses income registration safely (no action kind for receita)', () => {
    const r = interpretMutationCommand('Registre uma receita de 100 reais de teste hoje.', NOW);
    expect(r.type).toBe('income_unsupported');
  });
