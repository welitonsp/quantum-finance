...
import {
  Loader2, Dna, TrendingUp, TrendingDown, AlertTriangle,
  ShieldCheck, RefreshCw, Info, GitCompare,
  Scissors, PiggyBank, ArrowUpRight, ArrowDownRight,
} from 'lucide-react';
...
const DEFAULT_STATS: SimStats = {
  ...
  receitaTotalCents:   500000,
  despesaTotalCents:   420000,
};

const PRESETS = [
  { label: 'Trocar de Emprego',   icon: TrendingUp, aumentoSalario: 25, corteDespesas: 0  },
  { label: 'Cortar Restaurantes', icon: Scissors,   aumentoSalario: 0,  corteDespesas: 20 },
  { label: 'Modo Poupança',       icon: PiggyBank,  aumentoSalario: 0,  corteDespesas: 30 },
] as const;
...
function DnaCell({ label, value, sign, accent, isPrivacyMode, Icon }: DnaCardProps) {
  const prefix = sign === '=' ? '' : sign === '+' ? '+' : '−';
  return (
    <div className="bg-quantum-card/60 border border-white/8 rounded-xl px-4 py-3">
      ...
      <p className={`text-sm font-black font-mono ${accent}`}>
        {isPrivacyMode ? '•••••' : `${prefix}${fmtBRL(value)}`}
      </p>
    </div>
  );
}

export default function GemeloFinanceiro({
  recurringTasks, debts, creditCards, transactions, balances,
}: Props) {
  ...
  const histStats = useMemo<SimStats>(() => {
  ...
  const dna = useGemeloData({
    recurringTasks,
    debts,
    creditCards,
    transactions,
    historicalIncomeCents:  histStats.receitaTotalCents as Centavos,
    historicalExpenseCents: histStats.despesaTotalCents as Centavos,
  });
  ...
  const workerInputs = useMemo(() => {
    ...
    const mediaVariavelCents = Math.max(
      ...
      histStats.mediaVariavelCents - despesaFixaCents,
    );
    return {
      receitaMensalCents,
      despesaFixaCents,
      mediaVariavelCents: Math.max(0, mediaVariavelCents),
      desvioVariavelCents: histStats.desvioVariavelCents,
    };
  }, [dna, histStats]);
  ...
  const runSimulation = useCallback(() => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      workerRef.current!.postMessage({
        ...
        mediaVariavelCents:  workerInputs.mediaVariavelCents,
        desvioVariavelCents: workerInputs.desvioVariavelCents,
  ...
  }, [saldoCents, workerInputs, inflacao, corteDespesas, aumentoSalario, meses]);

  useEffect(() => { runSimulation(); }, [runSimulation]);

  const applyPreset = useCallback((preset: typeof PRESETS[number]) => {
    setAumentoSalario(preset.aumentoSalario);
    setCorteDespesas(preset.corteDespesas);
  }, []);
  ...
  const ProbIcon = prob === null ? Dna : prob >= 60 ? ShieldCheck : AlertTriangle;

  const netPositive = dna.netMonthlyCents >= 0;
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black text-quantum-fg tracking-tight">Gêmeo Financeiro</h1>
            <p className="text-xs text-quantum-fgMuted uppercase tracking-wider font-medium">Digital Twin · Compromissos reais · Monte Carlo</p>
          </div>
        </div>
        <button
      ...
      {/* Seção 1 — DNA Financeiro */}
      <div className="bg-quantum-bg/90 backdrop-blur-2xl border border-white/8 rounded-2xl p-6 space-y-4">
        <div className="flex items-center justify-between gap-3 pb-3 border-b border-quantum-border">
          <div className="flex items-center gap-2">
            <Dna className="w-4 h-4 text-cyan-400" />
            <h3 className="text-xs font-black text-quantum-fg uppercase tracking-widest">DNA Financeiro</h3>
          </div>
          <span className="text-[10px] font-mono text-quantum-fgMuted">
            {dna.sources.recurringCount} recorrentes · {dna.sources.activeDebtCount} dívidas · {dna.sources.cardCount} cartões
          </span>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          <DnaCell label="Renda Fixa"       value={dna.fixedIncomeCents}   sign="+" accent="text-emerald-400" isPrivacyMode={isPrivacyMode} Icon={ArrowUpRight} />
          <DnaCell label="Gastos Fixos"     value={dna.fixedExpensesCents} sign="-" accent="text-red-400"     isPrivacyMode={isPrivacyMode} Icon={ArrowDownRight} />
          <DnaCell label="Dívidas (PMT)"    value={dna.debtPaymentsCents}  sign="-" accent="text-orange-400"  isPrivacyMode={isPrivacyMode} Icon={ArrowDownRight} />
          <DnaCell label="Cartão (parc. futuras)" value={dna.cardCommittedCents} sign="-" accent="text-purple-400" isPrivacyMode={isPrivacyMode} Icon={ArrowDownRight} />
          <div className="col-span-2 bg-quantum-card/60 border border-white/8 rounded-xl px-4 py-3 flex flex-col justify-center">
            <span className="text-[9px] font-black text-quantum-fgMuted uppercase tracking-wider mb-1">Resultado Mensal Base</span>
            <div className="flex items-center gap-2">
              <p className={`text-lg font-black font-mono ${netPositive ? 'text-emerald-400' : 'text-rose-400'}`}>
                {isPrivacyMode ? '•••••' : `= ${fmtBRL(dna.netMonthlyCents)}`}
              </p>
              {netPositive
        ...
        <p className="text-[10px] text-quantum-fgMuted">
          Calculado dos seus compromissos reais — não de médias históricas.
        </p>
      </div>

      <div className="flex flex-col xl:flex-row gap-5">
        {/* Seção 2 — Ajustes + Presets */}
        <div className="xl:w-72 shrink-0 space-y-4">
          <div className="bg-quantum-bg/90 backdrop-blur-2xl border border-white/8 rounded-2xl p-5 space-y-5">
            <div className="flex items-center gap-2 pb-3 border-b border-quantum-border">
              ...
              <h3 className="text-xs font-black text-quantum-fg uppercase tracking-widest">Ajustes de Cenário</h3>
            </div>
          ...
          <div className="bg-quantum-bg/90 backdrop-blur-2xl border border-white/8 rounded-2xl p-5 space-y-3">
            <h3 className="text-[10px] font-black text-quantum-fgMuted uppercase tracking-widest pb-2 border-b border-quantum-border">Cenários Rápidos</h3>
            {PRESETS.map(preset => {
              ...
              return (
                <button
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2.5 bg-quantum-card/60 hover:bg-cyan-500/10 border border-white/8 hover:border-cyan-500/20 rounded-xl text-left transition-all group"
                >
                  <Icon className="w-4 h-4 text-cyan-400 group-hover:scale-110 transition-transform" />
                  <span className="text-xs font-bold text-quantum-fg">{preset.label}</span>
                </button>
              );
            })}
          </div>
        ...
        <div className="flex-1 space-y-4 min-w-0">
                ...
                <motion.div key="chart-ready" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }} className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={chartData} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                      <defs>
                        <linearGradient id="gemeloConeGradient" x1="0" y1="0" x2="0" y2="1">
                      ...
                      <Area type="monotone" dataKey="coneHeight" stackId="cone" fill="url(#gemeloConeGradient)"   stroke="none" legendType="none" isAnimationActive={false} />
          ...
          <p className="text-[10px] text-slate-600 text-center px-4 leading-relaxed">
            Projeção estocástica via transformação Box-Muller · 1.000 iterações independentes ·
            base construída dos seus compromissos reais (recorrentes, dívidas e parcelas de cartão).
