...
describe('🛡️ Fábrica de Formatação (UI)', () => {
  it('deve formatar valores monetários para o padrão BRL (R$)', () => {
    // Usamos .replace(/\s/g, ' ') para evitar falhas com espaços invisíveis do Intl
  ...
  it('deve formatar percentagens com a precisão exigida', () => {
    expect(formatPercent(15.55)).toBe('15.6%'); // Arredonda corretamente
    expect(formatPercent(10, 2)).toBe('10.00%'); // Força 2 casas decimais
