...
import { normalizeTotpCode } from '../shared/lib/mfa';

interface Props {
  onLogin: () => Promise<void>;
  /** Login interrompido por MFA — exibe o prompt de código TOTP. */
  mfaPending?: boolean;
  onSubmitMfaCode?: (code: string) => Promise<void>;
  onCancelMfa?: () => void;
}
...
export default function LoginScreen({ onLogin, mfaPending = false, onSubmitMfaCode, onCancelMfa }: Props) {
  ...
  const handleMfaSubmit = async () => {
    if (isLoading || !onSubmitMfaCode) return;
    if (!normalizeTotpCode(mfaCode)) {
      toast.error('Digite o código de 6 dígitos do seu app autenticador.');
      return;
    }
    setIsLoading(true);
    try {
      await onSubmitMfaCode(mfaCode);
      setMfaCode('');
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-quantum-fg flex flex-col items-center justify-center relative overflow-hidden font-sans">
      ...
      <div className="glass-card-dark p-10 max-w-md w-full mx-4 text-center relative z-10 border-t-4 border-t-cyan-500 shadow-2xl shadow-cyan-500/20">
        {mfaPending ? (
          <>
            <div className="w-20 h-20 mx-auto rounded-2xl bg-gradient-to-br from-indigo-500 to-cyan-500 flex items-center justify-center shadow-lg shadow-indigo-500/20 mb-6">
              <ShieldCheck className="w-10 h-10 text-quantum-fg" />
            </div>
            <h1 className="text-2xl font-black gradient-text tracking-wide uppercase mb-2">Verificação em duas etapas</h1>
            ...
            <input
              ...
              onKeyDown={(e) => { if (e.key === 'Enter') void handleMfaSubmit(); }}
              placeholder="000000"
            ...
            <button
              ...
              disabled={isLoading}
              className="w-full bg-gradient-to-r from-indigo-500 to-cyan-500 text-white font-bold px-6 py-4 rounded-xl hover:opacity-90 transition-opacity shadow-lg disabled:opacity-70 disabled:cursor-not-allowed mb-3"
            >
              {isLoading ? 'A verificar...' : 'Verificar código'}
            </button>
            <button
              onClick={() => { setMfaCode(''); onCancelMfa?.(); }}
              disabled={isLoading}
              className="w-full text-sm text-quantum-fgMuted hover:text-quantum-fg transition-colors disabled:opacity-70"
            >
              Cancelar e voltar
            </button>
          </>
