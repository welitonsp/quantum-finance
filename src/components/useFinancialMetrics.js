...
export function useFinancialMetrics(uid, transactions, currentMonth, currentYear) {
  ...
  const metrics = useMemo(() => {
    ...
    // 2. Património (Ativos e Passivos)
    let ativos = 0;
    let passivos = 0;
    ...
    // 3. Custo Fixo Mensal (Recorrentes)
    ...
    // Calcula quantos meses o utilizador sobrevive sem salário usando apenas ativos líquidos
    const gastoMensalMedio = despesa > custoFixoMensal ? despesa : (custoFixoMensal || 1); // Evita divisão por zero
