              ...
              {recurringTasks.map(item => (
                <div key={item.id} className="p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:bg-slate-800/30 transition-colors group">
                  ...
                  <div className="flex items-center gap-5 w-full sm:w-auto justify-between sm:justify-end">
                    <span className="font-mono text-base font-bold text-slate-300">{formatCurrency(Number(item.value))}</span>
                    <button
            ...
            <form onSubmit={(e) => void handleAddRecurring(e)} className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={newFrequency} onChange={e => setNewFrequency(e.target.value as 'mensal' | 'anual')} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-cyan-500 transition-colors">
