...
export default function OneTouchActionsCard({ recurringTasks }: Props) {
  ...
  const dueTasks = useMemo<DueTask[]>(() => {
    ...
    return recurringTasks
      .filter((task) => {
        ...
        return task.dueDay - dayOfMonth <= 7;
      })
      .map((task) => ({ task, daysUntilDue: task.dueDay - dayOfMonth }))
