...
const periods = [
  ...
  { label: '1A', days: 365, points: 52 }, // semanal
];

// Gera dados simulados com tendência de alta (Mock Data)
const generateData = (points, baseValue = 250000) => {
  const data = [];
  let value = baseValue;
  const step = 0.005; // variação percentual por ponto
...
export default function PortfolioChart() {
  const [activePeriod, setActivePeriod] = useState(periods[2]); // 1M por padrão
  const [chartData, setChartData] = useState(() => generateData(activePeriod.points));
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-quantum-card border border-quantum-border rounded-xl p-3 shadow-xl">
          <p className="text-quantum-fgMuted text-xs mb-1">{formatDateLabel(label, activePeriod)}</p>
      ...
      <div className="h-72 w-full min-h-[288px]">
        <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={288}>
          <LineChart data={chartData} margin={{ top: 5, right: 5, left: 0, bottom: 5 }}>
            ...
            <XAxis
              dataKey="name"
              tickFormatter={(val) => formatDateLabel(val, activePeriod)}
              stroke="#6B7A94"
