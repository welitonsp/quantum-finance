...
export default function ImportButton({ onImportTransactions, uid, existingTransactions }) {
  ...
  const processFile = async (file, password = null) => {
    ...
    try {
      ...
      if (extension === '.ofx') {
        ...
        toast.loading(password ? "A desencriptar PDF..." : "A descodificar PDF com Motor Quântico...", { id: toastId });
      ...
      toast.loading(`A processar ${transacoesProcessadas.length} registos...`, { id: toastId });

      // ✅ CORREÇÃO: Capturar o resultado (added, duplicates) da importação
      let resultado;
      ...
      if (resultado && typeof resultado === 'object') {
        ...
        if (added > 0 && duplicates > 0) {
          toast.success(`${added} transações adicionadas. ${duplicates} duplicadas ignoradas.`, { id: toastId });
        } else if (added > 0) {
          toast.success(`${added} transações importadas com sucesso!`, { id: toastId });
        } else if (duplicates > 0) {
          toast.warning(`Todas as ${duplicates} transações já existiam. Nenhuma duplicada foi adicionada.`, { id: toastId });
        } else {
          toast.success(`${transacoesProcessadas.length} transações importadas!`, { id: toastId });
        }
      } else {
        // Fallback caso o retorno não seja o esperado (por compatibilidade)
        toast.success(`${transacoesProcessadas.length} transações importadas!`, { id: toastId });
      }
      ...
      // Captura o erro específico de Senha do Banco
  ...
  return (
    <>
      ...
      >
        <Download className="w-4 h-4 mr-2 text-indigo-500 dark:text-indigo-400" /> Importar Extrato
      </button>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[20px] p-6 relative overflow-hidden">
              ...
              {status === 'loading' && (
                <div className="relative z-10 py-12 flex flex-col items-center text-center animate-in fade-in">
                  ...
                  <h4 className="text-sm font-bold text-slate-800 dark:text-white tracking-widest uppercase animate-pulse">A Processar...</h4>
                </div>
              )}
