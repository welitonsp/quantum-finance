...
const COMMANDER_COMMANDS: Omit<PaletteCommand, 'action'> & { action: () => void }[] = [
  {
    id: 'cmd-cut-lazer',
    label: 'Simular corte de 15% em lazer',
    description: 'IA projeta economia mensal com redução de gastos em lazer',
    icon: Scissors, shortcut: null, group: 'Comandante',
    action: () => {
  ...
  },
  {
    ...
    icon: HeartPulse, shortcut: null, group: 'Comandante',
    action: () => {
  ...
  },
  {
    ...
    icon: FileText, shortcut: null, group: 'Comandante',
    action: () => {
...
function buildCommands(
  ...
  return [
    ...
    { id: 'go-quantum',      label: 'Quantum AI',              description: 'Inteligência Artificial financeira',   icon: BrainCircuit, shortcut: null,    group: 'Navegar',     action: () => { navigate('quantum');   toast.success('Quantum AI'); } },
    {
      ...
      icon: isPrivacyMode ? Eye : EyeOff, shortcut: 'Alt+P', group: 'Preferências',
      action: () => { togglePrivacy(); toast.success(isPrivacyMode ? 'Privacidade desativada' : 'Privacidade ativada'); },
    },
  ];
}

const NAV_COMMAND_IDS = new Set(['go-dashboard','go-history','go-reports','go-wallet','go-accounts','go-cards','go-recurring','go-quantum']);

export default function CommandPalette({ isOpen, onClose, isCommanderMode = false }: CommandPaletteProps) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'ArrowDown') {
      ...
      if (cmd) {
        cmd.action();
        if (!NAV_COMMAND_IDS.has(cmd.id)) onClose();
      }
