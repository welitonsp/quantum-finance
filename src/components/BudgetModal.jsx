...
export default function BudgetModal({ isOpen, onClose, currentGoal, onSave }) {
  ...
  const [showConfirmZero, setShowConfirmZero] = useState(false); // ✅ CORREÇÃO: Estado para confirmação customizada
  ...
  const handleSubmit = (e) => {
    ...
    // ✅ CORREÇÃO: Em vez de window.confirm, ativamos a interface de confirmação
    if (numericGoal === 0) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-quantum-card w-full max-w-sm rounded-3xl p-6 shadow-2xl border border-quantum-border animate-in zoom-in-95 relative overflow-hidden">
        ...
        {/* ✅ INTERFACE DE CONFIRMAÇÃO CUSTOMIZADA */}
        {showConfirmZero ? (
          <div className="space-y-6 relative z-10 animate-in slide-in-from-right-4">
            ...
            <div className="flex gap-3">
              <button 
                ...
                className="flex-1 py-3 font-bold rounded-xl text-slate-300 bg-slate-800 hover:bg-slate-700 transition-colors text-sm"
              >
                Cancelar
              </button>
              <button 
                ...
                className="flex-1 py-3 font-bold rounded-xl text-white bg-red-600 hover:bg-red-700 transition-colors shadow-lg shadow-red-600/20 text-sm"
              >
                Confirmar
              </button>
            </div>
