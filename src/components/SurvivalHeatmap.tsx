...
const cellVariants = {
  ...
  show:   (i: number) => ({
    opacity: 1,
    scale:   1,
    transition: { delay: i * 0.012, type: 'spring' as const, stiffness: 320, damping: 26 },
  }),
};
...
function HeatTooltip({ day, spent, label, isPrivacyMode, anchorRect, containerRef }: TooltipProps) {
...
export default function SurvivalHeatmap({ transactions, currentMonth, currentYear }: Props) {
  ...
  const cells = useMemo<(DayData | null)[]>(() => {
          ...
          return (
            <motion.div
              ...
              onMouseEnter={(e) => {
                ...
                const mm   = currentMonth < 10 ? `0${currentMonth}` : String(currentMonth);
                setHovered({ day: `${cell.day}/${mm}`, spent: cell.spent, label, rect });
              }}
