    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-quantum-card w-full max-w-sm rounded-3xl p-6 shadow-2xl border border-quantum-border animate-in zoom-in-95 relative overflow-hidden">
        <div className="flex items-center justify-between mb-6 relative z-10">
          <h2 className="text-xl font-bold text-quantum-fg flex items-center gap-2"><Target className="w-5 h-5 text-quantum-accent" />Teto Mensal</h2>
          <button onClick={onClose} className="p-1.5 bg-quantum-bgSecondary text-quantum-fgMuted rounded-full hover:bg-quantum-border hover:text-quantum-fg transition-colors"><X className="w-5 h-5" /></button>
        </div>
        ...
        {showConfirmZero ? (
          <div className="space-y-6 relative z-10 animate-in slide-in-from-right-4">
            <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-2xl text-center">
              ...
              <h3 className="text-sm font-bold text-quantum-fg mb-1">Desativar Orçamento?</h3>
              <p className="text-xs text-quantum-fgMuted">Ao definir a meta como R$ 0.00, a IA deixará de monitorizar e avisar sobre os seus gastos.</p>
            </div>
            <div className="flex gap-3">
              <button type="button" onClick={() => setShowConfirmZero(false)} className="flex-1 py-3 font-bold rounded-xl text-quantum-fg bg-quantum-bgSecondary hover:bg-slate-700 transition-colors text-sm">Cancelar</button>
              <button type="button" onClick={() => executeSave(0)} className="flex-1 py-3 font-bold rounded-xl text-quantum-fg bg-red-600 hover:bg-red-700 transition-colors shadow-lg shadow-red-600/20 text-sm">Confirmar</button>
            </div>
          </div>
        ) : (
