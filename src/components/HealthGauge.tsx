...
interface Props {
  score: number;
  color: 'emerald' | 'amber' | 'red' | string;
}

export const HealthGauge = memo(({ score, color }: Props) => {
  const [animatedFill, setAnimatedFill] = useState(0);
  const r = 46, circ = 2 * Math.PI * r, arc = circ * 0.72;
  const fill = (Math.max(0, animatedFill) / 100) * arc;
  const C = ({ emerald: '#10b981', amber: '#f59e0b', red: '#ef4444' } as Record<string, string>)[color] || '#10b981';
  const lbl = color === 'red' ? 'CRÍTICO' : color === 'amber' ? 'ATENÇÃO' : score >= 80 ? 'EXCELENTE' : 'SAUDÁVEL';

  useEffect(() => {
    const t = setTimeout(() => setAnimatedFill(score), 200);
