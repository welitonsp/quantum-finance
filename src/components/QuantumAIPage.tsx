...
type KpiColorKey = 'green' | 'red' | 'gold' | 'purple';
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function groupByCategory(transactions: AnyRecord[]): CategoryTotal[] {
  const map: Record<string, number> = {};
  transactions.forEach(tx => {
    if (tx.type !== 'saida' && tx.type !== 'despesa') return;
    const cat = (tx.category as string) || 'Outros';
    map[cat] = (map[cat] ?? 0) + Math.abs(Number(tx.value || 0));
  });
  return Object.entries(map).sort((a, b) => b[1] - a[1]).map(([cat, total]) => ({ cat, total }));
}

function calcBurnMetrics(transactions: AnyRecord[], currentMonth: number, currentYear: number): BurnMetrics {
  const hoje     = new Date();
  ...
  return {
    despesasMes, receitasMes, ritmoDiario, projecaoFinal, saldoProjectado,
...
const COLOR_MAP: Record<KpiColorKey, { text: string; bg: string; border: string; shadow: string }> = {
...
// ─── FixedExpensesRisk ────────────────────────────────────────────────────────
interface FixedExpensesRiskProps {
  balances: Balances;
...
function AuditReportPanel({ transactions, balances, currentMonth, currentYear }: AuditReportPanelProps) {
  ...
  return (
    <div className="glass-card-quantum p-5">
      ...
      <AnimatePresence>
        {loading && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center py-10 gap-3">
        ...
        {!loading && report && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="prose prose-invert prose-sm max-w-none text-quantum-fg bg-quantum-bg/50 rounded-xl p-4 border border-quantum-border text-sm leading-relaxed whitespace-pre-wrap">
            {report}
          </motion.div>
        )}
        {!loading && !report && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center py-8 gap-3 text-center">
...
export default function QuantumAIPage({
  ...
  const burnColor:  KpiColorKey = burn.ritmoDiario === 0 ? 'green' : burn.saldoProjectado >= 0 ? 'gold' : 'red';
  const riscoColor: KpiColorKey = burn.projecaoFinal <= burn.receitasMes ? 'green' : 'red';
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-6 relative z-10">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard icon={Flame}       label="Ritmo Diário"         value={fmt(burn.ritmoDiario)}    sub={`${burn.diasRestantes} dias restantes`}      color={burnColor}  glow />
        <KpiCard icon={TrendingDown} label="Projeção Fim do Mês"  value={fmt(burn.projecaoFinal)}  sub={`Gasto atual: ${fmt(burn.despesasMes)}`}     color={riscoColor} glow />
