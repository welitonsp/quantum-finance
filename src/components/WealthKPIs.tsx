...
function getStatusColor(val: number, good: number, warn: number, isInverse = false): string {
  if (isInverse) {
    return val <= good
      ? 'text-quantum-accent bg-emerald-500/10'
      : val <= warn
        ? 'text-orange-400 bg-orange-500/10'
        : 'text-quantum-red bg-red-500/10';
  }
  return val >= good
    ? 'text-quantum-accent bg-emerald-500/10'
    : val >= warn
...
export default function WealthKPIs({ metrics, loading }: WealthKPIsProps) {
  if (loading) {
    return <div className="h-24 bg-quantum-card border border-quantum-border rounded-2xl animate-pulse" />;
  }
