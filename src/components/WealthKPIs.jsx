// src/components/WealthKPIs.jsx
import { PiggyBank, Scale, ShieldAlert, Target } from 'lucide-react';

export default function WealthKPIs({ metrics, loading }) {
  if (loading) {
    return <div className="h-24 bg-quantum-card border border-quantum-border rounded-2xl animate-pulse"></div>;
  }

  const { taxaPoupanca, endividamento, comprometimento, reservaMeses } = metrics;

  // Função auxiliar para definir as cores consoante a saúde da métrica
  const getStatusColor = (val, good, warn, isInverse = false) => {
    if (isInverse) {
  ...
  const kpis = [
    {
      ...
      desc: "Do salário guardado",
      icon: PiggyBank,
      colorClass: getStatusColor(taxaPoupanca, 20, 10, false),
      ideal: "Ideal: > 20%"
    },
    {
      title: "Índice de Dívida",
      value: `${endividamento.toFixed(1)}%`,
      desc: "Património comprometido",
      icon: Scale,
      colorClass: getStatusColor(endividamento, 30, 50, true),
      ideal: "Ideal: < 30%"
    },
    {
      title: "Comprometimento",
      value: `${comprometimento.toFixed(1)}%`,
      desc: "Salário vs Custos Fixos",
      icon: Target,
      colorClass: getStatusColor(comprometimento, 35, 50, true),
      ideal: "Ideal: < 35%"
    },
    {
      title: "Sobrevivência",
      value: `${reservaMeses.toFixed(1)} m`,
      desc: "Meses de Reserva",
      icon: ShieldAlert,
      colorClass: getStatusColor(reservaMeses, 6, 3, false),
      ideal: "Ideal: > 6 meses"
    }
  ];
    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
      {kpis.map((kpi, i) => (
        <div key={i} className="bg-quantum-card border border-quantum-border rounded-2xl p-4 md:p-5 relative overflow-hidden group hover:border-quantum-accent/50 transition-colors shadow-sm hover:shadow-[0_0_20px_rgba(0,230,138,0.05)]">
          ...
          <div className="absolute -right-6 -top-6 w-24 h-24 bg-white/5 rounded-full blur-2xl group-hover:bg-quantum-accent/10 transition-colors pointer-events-none"></div>
