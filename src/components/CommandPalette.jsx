/**
 * CommandPalette.jsx — Palete de Comandos Global (Ctrl+K / Cmd+K)
 * ──────────────────────────────────────────────────────────────────────────────
 * Modal glassmorphism com pesquisa de comandos, navegação por teclado e
 * execução de ações com feedback via toast.
 *
 * ATALHOS:
 *  • Ctrl+K / Cmd+K        → abre/fecha modo normal
 *  • Ctrl+Shift+K / Cmd+Shift+K → abre Modo Comandante (comandos exclusivos)
 *  • ↑ / ↓                 → navega entre resultados
 *  • Enter                 → executa o comando em foco
 *  • Esc                   → 1º limpa o campo de busca; 2º fecha (se campo vazio)
 */
...
// ─── Comandos Exclusivos do Modo Comandante ───────────────────────────────────
const COMMANDER_COMMANDS = [
  {
    ...
    action: () => {
      ...
      setTimeout(() => {
        toast.success('Simulação: corte de 15% em lazer liberta ~R$ 187/mês. Projeção anual: R$ 2.244 poupados.', {
  ...
  },
  {
    ...
    action: () => {
      ...
      setTimeout(() => {
        toast.success('Padrão detetado: gastos recorrentes às 3ª-feira (+43% acima da média). Categoria: Alimentação fora de casa.', {
  ...
  },
  {
    ...
    action: () => {
      ...
      setTimeout(() => {
        toast.success('Relatório de endividamento pronto! (Funcionalidade completa em breve)', {
          icon: '📊', duration: 5000,
        });
      }, 1300);
    },
...
const buildCommands = ({ navigate, togglePrivacy, isPrivacyMode }) => [
  {
    id: 'new-transaction',
    ...
    group: 'Ações',
    action: () => {
      toast.success('Abrindo formulário de transação…');
      navigate('dashboard');
    },
  },
  {
    ...
    shortcut: null,
    group: 'Navegar',
    action: () => { navigate('dashboard'); toast.success('Painel Central'); },
  },
  {
    id: 'go-history',
    label: 'Livro Razão',
    ...
    shortcut: null,
    group: 'Navegar',
    action: () => { navigate('history'); toast.success('Livro Razão'); },
  },
  {
    id: 'go-reports',
    label: 'Relatórios Analíticos',
    description: 'Gráficos e análises financeiras',
    icon: BarChart2,
    shortcut: null,
    group: 'Navegar',
    action: () => { navigate('reports'); toast.success('Relatórios Analíticos'); },
  },
  {
    id: 'go-wallet',
    label: 'Carteira',
    description: 'Visão consolidada dos ativos',
    icon: Wallet,
    shortcut: null,
    group: 'Navegar',
    action: () => { navigate('wallet'); toast.success('Carteira'); },
  },
  {
    id: 'go-accounts',
    label: 'As Minhas Contas',
    description: 'Gerir contas bancárias',
    icon: Wallet,
    shortcut: null,
    group: 'Navegar',
    action: () => { navigate('accounts'); toast.success('As Minhas Contas'); },
  },
  {
    id: 'go-cards',
    label: 'Cartões de Crédito',
    description: 'Gerir cartões e faturas',
    icon: CreditCard,
    shortcut: null,
    group: 'Navegar',
    action: () => { navigate('cards'); toast.success('Cartões de Crédito'); },
  },
  {
    id: 'go-recurring',
    label: 'Despesas Recorrentes',
    description: 'Gerir assinaturas e recorrências',
    icon: RefreshCw,
    shortcut: null,
    group: 'Navegar',
    action: () => { navigate('recurring'); toast.success('Despesas Recorrentes'); },
  },
  {
    id: 'go-quantum',
    label: 'Quantum AI',
    description: 'Inteligência Artificial financeira',
    icon: BrainCircuit,
    shortcut: null,
    group: 'Navegar',
    action: () => { navigate('quantum'); toast.success('Quantum AI'); },
  },
  {
    id: 'toggle-privacy',
    ...
    action: () => {
      togglePrivacy();
      toast.success(isPrivacyMode ? 'Privacidade desativada' : 'Privacidade ativada');
    },
  },
];

// ─── Componente Principal ─────────────────────────────────────────────────────
export default function CommandPalette({ isOpen, onClose, onNewTransaction, isCommanderMode = false }) {
  ...
  const allCommands = useMemo(() => {
    ...
    // Comandante: prepend comandos exclusivos + manter os normais para pesquisa completa
  ...
  // Resetar foco quando filtro muda
  ...
  // Scroll automático do item em foco
  useEffect(() => {
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'ArrowDown') {
      ...
      if (filtered[focusIdx]) {
        ...
        if (!['go-dashboard','go-history','go-reports','go-wallet','go-accounts','go-cards','go-recurring','go-quantum'].includes(filtered[focusIdx].id)) {
          onClose();
        }
      }
    } else if (e.key === 'Escape') {
      // [UX_ESC_KEY] 1º Esc limpa o campo; 2º Esc (campo vazio) fecha o modal
      if (query !== '') {
        e.stopPropagation(); // impede que App.jsx também processe este Esc
        setQuery('');
      } else {
        onClose();
      }
  ...
  // Índice global de cada comando (para highlight correto)
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div className={`mx-4 rounded-2xl overflow-hidden backdrop-blur-xl shadow-2xl shadow-black/60 ${
              isCommanderMode
                ? 'bg-slate-900/90 border border-violet-500/40 shadow-violet-900/30'
                : 'bg-slate-900/80 border border-white/10'
            }`}>

              {/* ── Badge Commander Mode ── */}
              {isCommanderMode && (
                <motion.div
              ...
              {/* ── Cabeçalho + Input ── [A11Y: combobox wraps input + listbox] */}
              <div
                role="combobox"
                aria-expanded={true}
                ...
                className={`flex items-center gap-3 px-4 py-3.5 border-b ${
                  isCommanderMode ? 'border-violet-500/20' : 'border-white/8'
                } ${isCommanderMode ? 'mt-2' : ''}`}
              >
                {isCommanderMode
                ...
                <input
                  ...
                  aria-activedescendant={
                    filtered.length > 0 ? `command-item-${focusIdx}` : undefined
                  }
                  value={query}
                  ...
                  className={`flex-1 bg-transparent text-white text-sm outline-none ${
                    isCommanderMode ? 'placeholder-violet-400/50' : 'placeholder-slate-500'
                  }`}
                  autoComplete="off"
                  spellCheck={false}
                />
                <div className="flex items-center gap-1.5 shrink-0">
                  <kbd className={`hidden sm:inline-flex items-center gap-1 px-2 py-1 rounded-lg border text-[10px] font-mono ${
                    isCommanderMode
              ...
              <div
                ...
                className="max-h-[360px] overflow-y-auto overscroll-contain custom-scrollbar py-2"
              >
                {filtered.length === 0 ? (
                  ...
                  Array.from(groups.entries()).map(([groupName, cmds]) => (
                    <div key={groupName}>
                      {/* Separador de grupo */}
                      <div className={`px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest flex items-center gap-1.5 ${
                        groupName === 'Comandante' ? 'text-violet-400' : 'text-slate-500'
                      }`}>
