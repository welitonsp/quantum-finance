...
import { useAppLogic } from "./hooks/useAppLogic"; // 🌟 O NOVO MOTOR LÓGICO
...
const AuthenticatedApp = ({ user }) => {
  ...
  // 🌟 O NOVO CÉREBRO: Toda a lógica complexa foi injetada a partir do Hook `useAppLogic`
  const {
  ...
  } = useAppLogic(user, displayedTransactions, activeModule, setCurrentMonth, setCurrentYear, setActiveModule, update, add, remove, removeBatch);
  ...
  return (
    <div className="flex h-screen overflow-hidden font-sans transition-colors duration-500 relative">
      <Toaster position="bottom-right" toastOptions={{ 
        style: { background: '#0F172A', color: '#F8FAFC', border: '1px solid #1E293B', borderRadius: '12px' },
        success: { iconTheme: { primary: '#00E68A', secondary: '#0F172A' } },
        error: { iconTheme: { primary: '#FF4757', secondary: '#0F172A' } }
      }} />
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-slate-900 w-full max-w-md rounded-3xl p-6 shadow-2xl border border-white/10 zoom-in-95">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div><h3 className="text-lg font-bold text-white">Apagar Registo?</h3><p className="text-sm text-slate-400">Esta ação não pode ser desfeita.</p></div>
            </div>
            <div className="bg-slate-950 p-3 rounded-xl mb-6 border border-white/5">
              <p className="text-sm font-bold truncate text-slate-300">"{transactionToDelete.description}"</p>
            </div>
            <div className="flex justify-end gap-3">
              <button onClick={() => setTransactionToDelete(null)} className="px-5 py-2.5 rounded-xl font-bold text-slate-400 hover:text-white hover:bg-white/5 transition-colors">Cancelar</button>
              <button onClick={confirmDelete} className="px-5 py-2.5 rounded-xl font-bold bg-red-600 text-white hover:bg-red-700 transition-colors shadow-lg shadow-red-500/20">Apagar</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="relative z-10 flex w-full h-full pointer-events-none">
        ...
        <div className="pointer-events-auto">
          <Sidebar user={user} isMobileMenuOpen={false} setIsMobileMenuOpen={() => {}} isSidebarCollapsed={isSidebarCollapsed} setIsSettingsOpen={setIsSettingsOpen} handleLogout={() => signOut(auth)} />
        </div>
        ...
        <div className="flex-1 flex flex-col w-full overflow-hidden pointer-events-auto bg-slate-950/80 backdrop-blur-sm">
          ...
          <Header 
            ...
            theme={theme} toggleTheme={toggleTheme} isSidebarCollapsed={isSidebarCollapsed} setIsSidebarCollapsed={setIsSidebarCollapsed} setIsMobileMenuOpen={() => {}} isFormOpen={isFormOpen} setIsFormOpen={setIsFormOpen} user={user} transactions={transactions} handleImport={handleImport} 
          />
            ...
            <div className="w-full max-w-[1600px] mx-auto p-4 md:p-6 lg:p-12 transition-all duration-300">
              ...
              <ErrorBoundary>
                <Suspense fallback={<PageLoader />}>
                  ...
                  {/* ✅ CORREÇÃO: Mercados agora isolado sem `alert` antigo */}
                  {currentPage === 'markets' && <MarketsPage />}
                  
                  {currentPage === 'quantum' && <QuantumAIPage />}
                  
                  {/* ✅ CORREÇÃO: Prop onBatchImport adicionada na HistoryPage */}
      ...
      <button onClick={() => setIsAIChatOpen(true)} className="fixed bottom-6 right-6 md:bottom-8 md:right-8 w-14 h-14 md:w-16 md:h-16 bg-cyan-600 hover:bg-cyan-500 rounded-2xl flex items-center justify-center shadow-[0_0_30px_rgba(8,145,178,0.4)] hover:scale-110 active:scale-95 transition-all z-50 group border border-white/20">
...
export default function App() {
  ...
  if (!authReady) return <div className="flex min-h-screen items-center justify-center bg-slate-950 text-cyan-400 font-bold uppercase animate-pulse">A inicializar o Quantum Finance...</div>;
