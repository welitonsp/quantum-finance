...
import {
  collection, addDoc, updateDoc, deleteDoc, doc, writeBatch, serverTimestamp,
...
export const FirestoreService = {
  ...
  // ─── TRANSAÇÕES GERAIS ──────────────────────────────────────────────────────
  async saveTransaction(uid: string, transactionData: AnyRecord): Promise<string> {
  ...
  async saveAllTransactions(uid: string, transactions: AnyRecord[]): Promise<{ added: number; duplicates: number; invalid: number }> {
    ...
    transactions.forEach((tx) => {
      ...
      batch.set(docRef, {
        ...
        createdAt: new Date(date as string).getTime(), updatedAt: serverTimestamp(),
      });
      added++;
    });
  ...
  async updateTransaction(id: string, data: AnyRecord): Promise<void> {
  ...
  async batchUpdateTransactions(ids: string[], updateData: AnyRecord): Promise<void> {
    if (!ids || ids.length === 0) return;
    const batch   = writeBatch(db);
    const payload = { ...updateData, updatedAt: serverTimestamp() };
    // Nunca permitir sobrescrita acidental de valor via batch
  ...
  async saveRecurringTransaction(uid: string, data: AnyRecord): Promise<string> {
    if (!uid) throw new Error('UID ausente.');
  ...
  async updateRecurringTransaction(id: string, data: AnyRecord): Promise<void> {
    if (!id) throw new Error('ID ausente.');
    const payload: AnyRecord = { ...data, updatedAt: serverTimestamp() };
    if (data['value'] !== undefined) payload['value'] = Math.round(Number(data['value']) * 100);
