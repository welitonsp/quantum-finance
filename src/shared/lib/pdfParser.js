...
export const parsePDF = async (file, password = null) => {
  try {
    ...
    try {
      ...
      const firstPageText = textContent.items.map(i => i.str).join(' ');
    ...
    for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
      ...
      for (const line of lines) {
        ...
        let [ , dataRaw, descricao, valorRaw, sufixo] = match;
        let valorNum = parseFloat(valorRaw.replace(/\s/g, '').replace(/\./g, '').replace(',', '.').replace(/^-/, ''));
        let isNegative = valorRaw.includes('-') || sufixo === 'D' || sufixo === '-';
    ...
    if (error.message === 'PASSWORD_REQUIRED') throw error;
