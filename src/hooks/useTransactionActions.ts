...
interface TransactionActionsParams {
  ...
  setTransactionToDelete:(tx: AnyRecord | null) => void;
}

export function useTransactionActions({
  user, update, add, remove, removeBatch,
  transactionToEdit, setTransactionToEdit, setIsFormOpen, setTransactionToDelete,
}: TransactionActionsParams) {
  const uid = user?.uid;

  const handleSaveTransaction = useCallback(async (data: AnyRecord) => {
    try {
      if (transactionToEdit) {
        await update(transactionToEdit['id'] as string, data);
  ...
  const confirmDelete = useCallback(async (transactionToDelete: AnyRecord | null) => {
    if (!transactionToDelete) return;
    const idToDelete = transactionToDelete['id'] as string;
