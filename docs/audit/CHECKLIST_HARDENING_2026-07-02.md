...
- [x] Migração de floats legados: **desbloqueio parcial e controlado**, autorizado explicitamente pelo owner. Novo `functions/scripts/executeLegacyMigrationSafe.js` habilita `--execute` real **só** para o subconjunto `migrationEligible` (já tem `value_cents` seguro; patch é só `{schemaVersion:2, source?}`, zero matemática monetária nova) — fail-closed em lote misto, exige backup validado. `executeLegacyMigration.js` não foi tocado (continua dry-run puro, guardrail estático intacto). Casos ambíguos/`adminRepairRequired` seguem bloqueados por decisão. Ver `docs/DECISOES-ARQUITETURA.md#migração-de-floats-legados--desbloqueio-parcial-e-controlado-fase-10d-rodada-2`. **Pendente do owner:** rodar `diagnoseLegacyTransactions.js` (read-only) contra produção — não sabemos se existe algum documento `migrationEligible` de fato.
